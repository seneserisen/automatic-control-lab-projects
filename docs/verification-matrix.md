...
This matrix connects engineering claims to automated evidence across MATLAB, Python, and C.
...
| MAG-001 | The local magnetic-levitation model must be observable from gap and coil-current measurements. | Observability rank = 3 | `TestMaglevObserver/modelIsObservable` | `test_maglev_observer_design_is_stable_and_observable` | Uses the validated precomputed observer gain; design verification remains in MATLAB/Python |
| MAG-002 | Controller and observer error dynamics must be asymptotically stable. | Pole real parts < 0 | `TestMaglevObserver/controllerAndObserverAreStable` | `test_maglev_observer_design_is_stable_and_observable` | Runtime tracking and bounded-state tests verify the deployed gains behave stably in the committed scenario |
...
| MAG-008 | The deployment-oriented runtime must avoid dynamic memory and use fixed-size state storage. | Static source inspection and compiler build | Not applicable | Not applicable | C API and source use only stack/static-size objects |
...
| DOC-001 | Published metrics must match the current executable reference models. | `docs/results-summary.md` | Not applicable | `python -m validation.report --check` | C demo prints independently reproducible runtime metrics |
| DOC-002 | Uploaded laboratory documents must map to public code, tests, results, limitations, and attribution. | `docs/source-reconstruction-map.md` | Project documentation | Project documentation | Project documentation |
...
| CI-002 | MATLAB utility and observer tests must pass in a real MATLAB runtime. | GitHub Actions JUnit artifact | Official MATLAB workflow | Not applicable | Not applicable |
...
A passing test verifies the documented simulation or runtime requirement under the committed model and configuration. It does not establish hardware safety, real-time schedulability, fixed-point correctness, robust stability across untested parameter ranges, MISRA-C compliance, or functional-safety certification.
