...
arguments
    ...
    end_time (1,1) double {mustBeFinite} = inf
end

if numel(time) ~= numel(error_signal)
    error('control_lab:SizeMismatch', 'time and error_signal must have equal lengths.');
end
if numel(time) < 2
    error('control_lab:NotEnoughSamples', 'At least two time samples are required.');
end

dt = median(diff(time));
window = max(1, ceil(hold_time/dt));
inside = time >= start_time & time <= end_time & abs(error_signal) <= tolerance;
run_length = 0;
entry_time = NaN;
