...
def _sample(x: np.ndarray, y: np.ndarray, maximum: int = 25) -> tuple[np.ndarray, np.ndarray]:
...
def _write_line_chart(
    ...
    for fraction in np.linspace(0, 1, 6):
        ...
        elements.append(
            f'<line x1="{x:.1f}" y1="{MARGIN_TOP}" x2="{x:.1f}" y2="{MARGIN_TOP + plot_h}" stroke="#ddd"/>'
        )
        elements.append(
            f'<line x1="{MARGIN_LEFT}" y1="{y:.1f}" x2="{MARGIN_LEFT + plot_w}" y2="{y:.1f}" stroke="#ddd"/>'
        )
        xv = xlim[0] + fraction * (xlim[1] - xlim[0])
        yv = ylim[1] - fraction * (ylim[1] - ylim[0])
        elements.append(
            f'<text x="{x:.1f}" y="{MARGIN_TOP + plot_h + 22}" text-anchor="middle" font-family="Arial" font-size="12">{xv:.2g}</text>'
        )
        elements.append(
            f'<text x="{MARGIN_LEFT - 10}" y="{y + 4:.1f}" text-anchor="end" font-family="Arial" font-size="12">{yv:.2g}</text>'
        )

    for _name, x, y, colour in sampled:
        elements.append(
            f'<polyline points="{_polyline(x, y, xlim, ylim)}" fill="none" stroke="{colour}" stroke-width="2.2"/>'
        )

    elements.append(
        f'<text x="{WIDTH / 2}" y="{HEIGHT - 14}" text-anchor="middle" font-family="Arial" font-size="14">{escape(xlabel)}</text>'
    )
    elements.append(
        f'<text x="18" y="{HEIGHT / 2}" text-anchor="middle" transform="rotate(-90 18 {HEIGHT / 2})" font-family="Arial" font-size="14">{escape(ylabel)}</text>'
    )
    ...
    for idx, (name, _, _, colour) in enumerate(sampled):
        y = legend_y + idx * 22
        elements.append(
            f'<line x1="{legend_x}" y1="{y}" x2="{legend_x + 28}" y2="{y}" stroke="{colour}" stroke-width="3"/>'
        )
        elements.append(
            f'<text x="{legend_x + 36}" y="{y + 4}" font-family="Arial" font-size="13">{escape(name)}</text>'
        )
    elements.append("</svg>")
...
def save_nonlinear(root: Path) -> None:
    ...
    for idx, (label, value, colour) in enumerate(zip(labels, values, colours, strict=True)):
        ...
        elements.append(
            f'<rect x="{cx - bar_w / 2:.1f}" y="{top:.1f}" width="{bar_w}" height="{height:.1f}" fill="{colour}"/>'
        )
        elements.append(
            f'<text x="{cx:.1f}" y="{HEIGHT - 45}" text-anchor="middle" font-family="Arial" font-size="13">{escape(label)}</text>'
        )
        elements.append(
            f'<text x="{cx:.1f}" y="{value_y - 8 if value >= 0 else value_y + 18:.1f}" text-anchor="middle" font-family="Arial" font-size="14">{value:.2f}</text>'
        )
    elements.append(
        f'<text x="20" y="{HEIGHT / 2}" text-anchor="middle" transform="rotate(-90 20 {HEIGHT / 2})" font-family="Arial" font-size="14">Maximum pole real part [1/s]</text>'
    )
    elements.append("</svg>")
...
def save_rotary(root: Path) -> None:
    ...
    _write_line_chart(
        ...
        [
            ...
            ("Feedback only", result.time, result.feedback_only_response, "#c23b22"),
            ("2-DOF", result.time, result.two_dof_response, "#2a6fbb"),
        ],
    )
...
def save_suspension(root: Path) -> None:
    ...
    _write_line_chart(
        ...
        "Acceleration [m/s²]",
        [
            ("Passive", result.time, result.passive_acceleration, "#c23b22"),
            ("Active", result.time, result.active_acceleration, "#2a6fbb"),
        ],
    )
...
def save_maglev(root: Path) -> None:
    ...
    _write_line_chart(
        ...
        "Gap deviation [mm]",
        [
            ("Reference", result.time, 1000 * result.reference, "#444444"),
            ("Linear model", result.time, 1000 * result.linear_position, "#c23b22"),
            ("Nonlinear plant", result.time, 1000 * result.nonlinear_position, "#2a6fbb"),
        ],
    )
...
def numerical_checks() -> None:
    ...
    assert nonlinear.controllability_rank == 2
    ...
    assert nonlinear.near_model_rmse < nonlinear.far_model_rmse

    rotary = rotary_arm_simulation()
    assert abs(rotary.two_dof_response[-1] - 1.0) < 1e-3
    assert rotary.two_dof_rmse < rotary.feedback_only_rmse
    assert rotary.maximum_two_dof_control <= 25.0 + 1e-12

    suspension = active_suspension_simulation()
    assert suspension.active_acceleration_rms < suspension.passive_acceleration_rms
    assert suspension.peak_active_force <= 50.0 + 1e-12
    ...
    assert abs(maglev.nonlinear_position[-1] - 0.0005) < 2e-5
    assert maglev.linear_nonlinear_rmse < 2e-6
    assert maglev.maximum_voltage <= 30.0 + 1e-12

    tanks = two_tank_simulation()
    assert tanks.maximum_steady_state_level < 35.0
    assert np.isfinite(tanks.no_anti_windup.recovery_time)
    assert np.isfinite(tanks.anti_windup.recovery_time)
    assert tanks.anti_windup.recovery_time < tanks.no_anti_windup.recovery_time
